function plot_dist_to_safe_probes_local(dist_to_safe_summary, protocol, varargin)
% PLOT_DIST_TO_SAFE_PROBES_LOCAL  Probe-trial distance-to-safe for place learning protocols
%
%   Generates ONE SEPARATE FIGURE PER GENOTYPE.
%   Plots mean distance-to-safe for probe trials (PP + B1.P-B4.P) with errorbar.
%
%   Applies to: P003, P005, P006, P007, P008, P009
    ...
    if ~cfg.is_place_learning
        fprintf('  %s: Not a place learning protocol — skipping probe dist-to-safe plot.\n', protocol);
        return;
    end
    ...
    switch METRIC
        case 'mean_dist_to_safe_mm'
            y_label = 'Mean Distance to Safe — First Entry (mm)';  plot_suffix = 'dist_to_safe_probes';
        case 'median_dist_to_safe_mm'
            y_label = 'Median Distance to Safe — First Entry (mm)';  plot_suffix = 'dist_to_safe_median_probes';
        case 'mean_dist_to_safe_last_mm'
            y_label = 'Mean Distance to Safe — Last Entry (mm)';  plot_suffix = 'dist_to_safe_last_probes';
        case 'median_dist_to_safe_last_mm'
            y_label = 'Median Distance to Safe — Last Entry (mm)';  plot_suffix = 'dist_to_safe_last_median_probes';
        otherwise
            y_label = strrep(METRIC, '_', ' ');  plot_suffix = 'dist_to_safe_probes';
    end
    ...
    for gi = 1:length(geno_order)
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved probe dist-to-safe: %s\n', out_file);
        end
    ...
    if nGenos > 0
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved probe dist-to-safe overlay: %s\n', out_file);
        end
