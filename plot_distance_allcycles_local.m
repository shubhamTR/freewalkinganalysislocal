function plot_distance_allcycles_local(distance_summary, protocol, varargin)
% PLOT_DISTANCE_ALLCYCLES_LOCAL  Per-cycle distance for place learning protocols
%
%   Generates ONE SEPARATE FIGURE PER GENOTYPE.
%   Uses errorbar() with training-block shading, pre-train annotation,
%   and arena diameter line.  PP and probes are excluded (see probes plotter).
%
...
%     'ArenaDiameterMM'— arena diameter reference line in mm (default: 150)
    ...
    addParameter(p, 'ArenaDiameterMM', 150, @isnumeric);
    ...
    if ~cfg.is_place_learning
        fprintf('  %s: Not a place learning protocol — skipping all-cycles distance plot.\n', protocol);
        return;
    end
    ...
    ARENA_DIAM = opts.ArenaDiameterMM;
    ...
    for gi = 1:nGenos
        ...
        % Arena diameter reference line
        if ARENA_DIAM > 0
            hRef = yline(ax, ARENA_DIAM, '-', 'Color', [0.6 0.6 0.6], 'LineWidth', 1);
            set(hRef, 'HandleVisibility', 'off');
            text(num_cycles + 0.3, ARENA_DIAM, ...
                sprintf('Arena diameter (%d mm)', ARENA_DIAM), ...
        ...
        if ~isempty(opts.SavePath)
            ...
            saveas(fig, out_file);
            fprintf('  Saved distance all-cycles: %s\n', out_file);
        end
    ...
    if nGenos > 0
        ...
        % Arena diameter reference line
        if ARENA_DIAM > 0
            hRef = yline(ax_ov, ARENA_DIAM, '-', 'Color', [0.6 0.6 0.6], 'LineWidth', 1);
            set(hRef, 'HandleVisibility', 'off');
            text(num_cycles + 0.3, ARENA_DIAM, ...
                sprintf('Arena diameter (%d mm)', ARENA_DIAM), ...
                'FontSize', 8, 'Color', [0.5 0.5 0.5], 'VerticalAlignment', 'bottom');
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved distance all-cycles overlay: %s\n', out_file);
        end
