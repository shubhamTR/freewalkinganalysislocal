function plot_latency_allcycles_local(latency_summary, protocol, varargin)
% PLOT_LATENCY_ALLCYCLES_LOCAL  Per-cycle latency for place learning protocols
%
%   Generates ONE SEPARATE FIGURE PER GENOTYPE.
%   Uses errorbar() with training-block shading and pre-train annotation.
...
%     'YMax'        — fixed y-axis maximum (default: auto)
    ...
    if ~cfg.is_place_learning
        fprintf('  %s: Not a place learning protocol — skipping all-cycles latency plot.\n', protocol);
        return;
    end
    ...
    switch METRIC
        case 'mean_latency_s'
            y_label = 'Latency to dark quadrant (s)';
            plot_suffix = 'lat_allcycles';
        case 'median_latency_s'
            y_label = 'Median latency to dark quadrant (s)';
            plot_suffix = 'lat_median_allcycles';
        otherwise
    ...
    for gi = 1:nGenos
        ...
        if ~isempty(opts.SavePath)
            ...
            saveas(fig, out_file);
            fprintf('  Saved latency all-cycles: %s\n', out_file);
        end
    ...
    if nGenos > 0
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved latency all-cycles overlay: %s\n', out_file);
        end
