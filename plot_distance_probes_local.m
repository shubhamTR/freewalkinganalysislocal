function plot_distance_probes_local(distance_summary, protocol, varargin)
% PLOT_DISTANCE_PROBES_LOCAL  Probe-trial distance for place learning protocols
%
%   Generates ONE SEPARATE FIGURE PER GENOTYPE.
%   Plots mean distance for probe trials (PP + B1.P-B4.P) with errorbar.
%
%   Applies to: P003, P005, P006, P007, P008, P009
    ...
    if ~cfg.is_place_learning
        fprintf('  %s: Not a place learning protocol — skipping probe distance plot.\n', protocol);
        return;
    end
    ...
    switch METRIC
        case 'mean_dist_mm'
            y_label = 'Mean Distance (mm)';  plot_suffix = 'dist_probes';
        otherwise
            y_label = strrep(METRIC, '_', ' ');  plot_suffix = 'dist_probes';
    end
    ...
    for gi = 1:length(geno_order)
        ...
        if ~isempty(opts.SavePath)
            ...
            saveas(fig, out_file);
            fprintf('  Saved probe distance: %s\n', out_file);
        end
    ...
    if nGenos > 0
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved probe distance overlay: %s\n', out_file);
        end
