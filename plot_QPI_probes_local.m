function plot_QPI_probes_local(protocol, varargin)
% PLOT_QPI_PROBES_LOCAL  Probe-trial QPI for place learning protocols
%
%   plot_QPI_probes_local('P017')
%   plot_QPI_probes_local('P017', 'ShowPlots', true)
%
%   Generates ONE SEPARATE FIGURE PER GENOTYPE.
%   Plots mean |QPI| for probe trials (PP + B1.P-B4.P) with errorbar.
%
...
%     'Metric'       — 'mean_QPI_firsthalf' or 'mean_QPI_secondhalf'
    ...
    if ~cfg.is_place_learning
        fprintf('  %s: Not a place learning protocol — skipping probe QPI plot.\n', protocol);
        return;
    end
    ...
    for gi = 1:length(geno_order)
        ...
        set(ax, 'FontSize', 16);
        xlabel('Probe Trial', 'FontSize', 18);
        ...
        if ~isempty(opts.SavePath)
            out_file = fullfile(opts.SavePath, sprintf('QPI_probes_%s_%s_%s.png', protocol, half_suffix, geno));
            exportgraphics(fig, out_file, 'Resolution', 200);
            saveas(fig, strrep(out_file, '.png', '.svg'));
            savefig(fig, strrep(out_file, '.png', '.fig'));
            fprintf('  Saved probe QPI: %s\n', out_file);
        end
    ...
    if nGenos > 0
        ...
        if ~isempty(opts.SavePath)
            out_file = fullfile(opts.SavePath, sprintf('QPI_probes_%s_%s_overlay.png', protocol, half_suffix));
            exportgraphics(fig_ov, out_file, 'Resolution', 200);
            saveas(fig_ov, strrep(out_file, '.png', '.svg'));
            savefig(fig_ov, strrep(out_file, '.png', '.fig'));
            fprintf('  Saved probe QPI overlay: %s\n', out_file);
        end
