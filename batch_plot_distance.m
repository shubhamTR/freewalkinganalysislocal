%% batch_plot_distance.m
% Generate per-experiment distance plots for all experiments, plus a
% per-protocol summary plot (mean +/- SE across experiments with scatter).
%
% Calls plot_distance_per_experiment_local for each experiment, which
% computes distance internally and saves distance_<exp_name>.png to analysis/.
...
for p = 1:length(protocol_dirs)
    ...
    for e = 1:length(exp_dirs)
        ...
        if ~FORCE_REPLOT && ~isempty(dir(fullfile(analysis_subdir, sprintf('distance_%s.png', exp_name))))
            fprintf('  [%d/%d] %s — distance plot exists, skipping\n', e, length(exp_dirs), exp_name);
            nSkipped = nSkipped + 1;
            continue;
        end
        ...
        try
            ...
            s = plot_distance_per_experiment_local(exp_path, ...
                'Protocol', prot_name, 'PixelsPerMM', PIXELS_PER_MM, ...
                'ShowPlots', false, 'SavePlot', true);
            if ~isempty(s)
    ...
    if length(prot_summaries) >= 1
        try
            plot_distance_summary(prot_summaries, prot_name, prot_path);
        catch ME
...
%% Summary
fprintf('\n========================================\n');
fprintf('DISTANCE BATCH PLOT SUMMARY\n');
fprintf('========================================\n');
fprintf('Plotted: %d\n', nPlotted);
fprintf('Skipped: %d\n', nSkipped);
fprintf('Failed:  %d\n', nFailed);
...
function plot_distance_summary(summaries, prot_name, prot_path)
% PLOT_DISTANCE_SUMMARY  Per-genotype summary: mean +/- SE across experiments
%   with individual fly scatter and experiment means.
    ...
    for gi = 1:length(genotypes)
        ...
        % Collect per-experiment mean distances and fly data
        ...
        for k = 1:nExp
            ...
            dist_mat = summaries{ei}.distance_per_fly;

            om_m = strcmp(ct.label, 'OM1') | strcmp(ct.label, 'OM2');
            ct = ct(~om_m, :);
            dist_mat = dist_mat(:, ~om_m);

            for ci = 1:nCycles
                idx = find(strcmp(ct.label, ref_labels{ci}), 1);
                if ~isempty(idx)
                    exp_means(k, ci) = ct.mean_dist_mm(idx);
                    fly_vals = dist_mat(:, idx);
        ...
        ylabel(ax, 'Distance (mm)', 'FontSize', 12);
        title(ax, sprintf('%s — %s — Distance Summary (n=%d experiments)', ...
        ...
        %% Save
        out_png = fullfile(summary_dir, sprintf('summary_distance_%s_%s.png', prot_name, geno));
        out_fig = fullfile(summary_dir, sprintf('summary_distance_%s_%s.fig', prot_name, geno));
        exportgraphics(fig, out_png, 'Resolution', 150);
        savefig(fig, out_fig);
        close(fig);
        fprintf('  Saved summary distance plot: %s\n', out_png);
    end
end
