function plot_latency_blocks_local(latency_summary, protocol, varargin)
% PLOT_LATENCY_BLOCKS_LOCAL  Block-mean training latency for place learning protocols
%
%   Generates ONE SEPARATE FIGURE PER GENOTYPE.
%   Plots mean (or median) latency per training block (B1-B4) with errorbar.
%
%   Applies to: P003, P005, P006, P007, P008, P009
    ...
    if ~cfg.is_place_learning
        fprintf('  %s: Not a place learning protocol — skipping block latency plot.\n', protocol);
        return;
    end
    ...
    switch METRIC
        case 'mean_latency_s'
            y_label = 'Mean Latency (s)';  plot_suffix = 'lat_blocks';
        case 'median_latency_s'
            y_label = 'Median Latency (s)';  plot_suffix = 'lat_median_blocks';
        otherwise
            y_label = strrep(METRIC, '_', ' ');  plot_suffix = 'lat_blocks';
    end
    ...
    if nGenos > 0
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved block latency overlay: %s\n', out_file);
        end
