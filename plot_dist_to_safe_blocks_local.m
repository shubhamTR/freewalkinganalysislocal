function plot_dist_to_safe_blocks_local(dist_to_safe_summary, protocol, varargin)
% PLOT_DIST_TO_SAFE_BLOCKS_LOCAL  Block-mean distance-to-safe for place learning protocols
%
%   Generates ONE SEPARATE FIGURE PER GENOTYPE.
%   Plots mean distance-to-safe per training block (B1-B4) with errorbar.
%
%   Applies to: P003, P005, P006, P007, P008, P009
    ...
    if ~cfg.is_place_learning
        fprintf('  %s: Not a place learning protocol — skipping block dist-to-safe plot.\n', protocol);
        return;
    end
    ...
    switch METRIC
        case 'mean_dist_to_safe_mm'
            y_label = 'Mean Distance to Safe — First Entry (mm)';  plot_suffix = 'dist_to_safe_blocks';
        case 'median_dist_to_safe_mm'
            y_label = 'Median Distance to Safe — First Entry (mm)';  plot_suffix = 'dist_to_safe_median_blocks';
        case 'mean_dist_to_safe_last_mm'
            y_label = 'Mean Distance to Safe — Last Entry (mm)';  plot_suffix = 'dist_to_safe_last_blocks';
        case 'median_dist_to_safe_last_mm'
            y_label = 'Median Distance to Safe — Last Entry (mm)';  plot_suffix = 'dist_to_safe_last_median_blocks';
        otherwise
            y_label = strrep(METRIC, '_', ' ');  plot_suffix = 'dist_to_safe_blocks';
    end
    ...
    for gi = 1:length(geno_order)
        ...
        blk_labels = arrayfun(@(b) sprintf('B%d', b), 1:num_blocks, 'UniformOutput', false);
        set(ax, 'XTick', 1:num_blocks, 'XTickLabel', blk_labels);
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved block dist-to-safe: %s\n', out_file);
        end
    ...
    if nGenos > 0
        ...
        blk_labels_ov = arrayfun(@(b) sprintf('B%d', b), 1:num_blocks, 'UniformOutput', false);
        set(ax_ov, 'XTick', 1:num_blocks, 'XTickLabel', blk_labels_ov);
        xlabel('Training Block', 'FontSize', 11);
        ylabel(y_label, 'FontSize', 11);
        ...
        if ~isempty(opts.SavePath)
            ...
            fprintf('  Saved block dist-to-safe overlay: %s\n', out_file);
        end
